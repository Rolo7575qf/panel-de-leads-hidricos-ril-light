...
function getGeminiClient(): GoogleGenAI {
  if (!aiClient) {
    ...
    if (!apiKey) {
      throw new Error('GEMINI_API_KEY is not defined. Add it to Vercel environment variables.');
    }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!comuna) {
      return res.status(400).json({ error: 'La comuna es requerida para contextualizar.' });
    }
...
`;
    ...
    return res.json({ text: messageText });
  } catch (error: any) {
    console.error('Error in /api/gemini/suggest-followup:', error);
    return res.status(500).json({
      error: error.message || 'Error interno del servidor generando el seguimiento.',
    });
  }
}
